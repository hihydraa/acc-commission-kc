...
- **กระนวน** (`src/branches/kranuan.ts`) — สร้างจากสเปค v2 ที่ยืนยันกับ PDF ต้นทางจริงแล้ว + ไฟล์อ้างอิง
  ...
  ระยะทางที่หาย/เกิน 209 กม. ต้อง **block** แถวนั้นไว้รอตรวจสอบแทนที่จะ default เป็น 0 แบบสามทอง (ดู
  `BranchConfig.freightMissingBehavior`), และการหักหนี้ค้างชำระ**ไม่หักอัตโนมัติ** — คำนวณเต็มจำนวนก่อนแล้ว
  แจ้งเตือนบัญชีให้พิจารณาหัก 50%/100% เอง (คอลัมน์ตั้งต้น = 0 เสมอ ต่างจากสามทองที่หักเต็มจำนวนอัตโนมัติ —
  ดู `BranchConfig.debtDeductionMode`) — ทั้งหมดนี้ยืนยันจากไฟล์อ้างอิงที่อนุมัติแล้วของกระนวนเอง ไม่ใช่การเดา
