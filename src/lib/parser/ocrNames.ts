...
export async function ocrCustomerNamesFromMasterPdf(buffer: Buffer): Promise<OcrNameResult> {
  ...
  let createCanvas: (w: number, h: number) => { getContext: (kind: "2d") => unknown; toBuffer: (mime: string) => Buffer };
  try {
    // pdfjs-dist's renderer calls ctx.fill(path)/ctx.stroke(path) with a
    // Path2D object in a way that needs @napi-rs/canvas's node-canvas
    // compat shim (node-canvas.js, bundled inside the package) — the raw
    // Canvas/getContext API throws "Value is none of these types String,
    // Path" without it, and the plain `canvas` (node-canvas) package
    // silently renders every glyph as blank instead (table borders draw
    // fine, all text is empty) — both dead ends found the hard way.
    //
    ...
    ({ createCanvas } = require("@napi-rs/canvas/node-canvas.js"));
  } catch (err) {
    warnings.push(`[OCR ชื่อลูกค้า] ไม่พบโมดูล canvas สำหรับ render PDF — ข้ามการแก้ไขชื่อด้วย OCR (${err instanceof Error ? err.message : String(err)})`);
  ...
  try {
    ...
    try {
      ...
      // (~1MB) downloads fresh on every cold start since /tmp doesn't
      // persist between invocations, but that's an acceptable tradeoff for
      // a feature scoped to one small file.
